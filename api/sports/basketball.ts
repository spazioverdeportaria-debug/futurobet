// Vercel Serverless Function for Basketball (NBA) matches
export default async function handler(req: any, res: any) {
  ...
  try {
    const [resNext, resPast] = await Promise.all([
      fetch('https://www.thesportsdb.com/api/v1/json/123/eventsnextleague.php?id=4387').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('https://www.thesportsdb.com/api/v1/json/123/eventspastleague.php?id=4387').then(r => r.ok ? r.json() : null).catch(() => null),
    ]);
